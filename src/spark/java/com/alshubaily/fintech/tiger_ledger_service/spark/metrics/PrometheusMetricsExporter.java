...
public class PrometheusMetricsExporter {
    ...
    private static final Counter transactionAmount =
            Counter.build()
    ...
    public static void export(Row row) {
        ...
        System.out.printf(
                "Exporting metrics: type=%s, count=%d, amount=%.2f%n", type, count, amount);

        transactionCounter.labels(type).inc(count);
        System.out.printf(
                "Incremented transaction_count_total{transaction_type=\"%s\"} by %d%n",
                type, count);

        transactionAmount.labels(type).inc(amount);
        System.out.printf(
                "Incremented transaction_amount_total{transaction_type=\"%s\"} by %.2f%n",
                type, amount);
    }
}
